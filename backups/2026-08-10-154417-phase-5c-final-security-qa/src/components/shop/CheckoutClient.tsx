...
export function CheckoutClient() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!result.ok) {
      setErrors({ cart: result.error });
      return;
    }

    clearCart();
    window.sessionStorage.removeItem(CHECKOUT_STORAGE_KEY);
    router.push(`/pedido/${result.orderNumber}?token=${result.lookupToken}`);
  }
  ...
  return (
    <section className="section-shell checkout-page">
      <header className="catalog-hero checkout-hero">
        <div>
          <span className="catalog-eyebrow">CHECKOUT {commerceConfig.currency}</span>
          <h1>Finaliza tu pedido</h1>
          <p>Crearemos tu pedido en modo demo/pending sin cargos reales.</p>
        </div>
      </header>
      ...
      <form className="checkout-layout" onSubmit={handleSubmit} noValidate>
        <div className="checkout-flow">
          ...
          <CheckoutStep number="3" title="Método de envío">
            ...
            <button className="button-primary checkout-submit" type="submit" disabled={submitting}>
              {submitting ? "Creando pedido..." : "Crear pedido de prueba"}
            </button>
          </CheckoutStep>
        </div>
