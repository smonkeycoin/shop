...
import { Activity, ArrowRight, PackagePlus } from "lucide-react";
    ...
    <article className={`bundle-card ${compact ? "compact" : ""}`}>
      <Link href={`/kits/${bundle.slug}`} className="bundle-card-main">
        <div className="bundle-image">
          <div className="bundle-product-stack">
            {visibleProducts.length > 0 ? (
              visibleProducts.slice(0, 4).map(({ product, asset }) => (
                <span key={product.id}>
                  <Image src={asset?.localPath ?? ""} alt="" fill sizes="120px" />
                </span>
              ))
            ) : (
              <span className="bundle-abstract-visual">
