          ...
          <div className="bundle-detail-stack">
            {products.slice(0, 5).map((product) => {
              ...
              return asset?.localPath ? (
                <span key={product.id}>
                  <Image src={asset.localPath} alt={product.name} fill sizes="180px" priority={product.id === products[0]?.id} />
                </span>
              ) : (
                <span className="pending-mini" key={product.id}>SKU pendiente</span>
              );
            })}
          </div>
        </div>
