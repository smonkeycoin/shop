...
export function ProductDetail({ product }: ProductDetailProps) {
  ...
  const primaryImage = asset?.localPath ?? product.images[0];
  const [selectedImage, setSelectedImage] = useState<string | undefined>(primaryImage);
  ...
  const galleryImages = [primaryImage, ...product.images.filter((image) => image !== primaryImage)].filter(
    Boolean,
  ) as string[];
