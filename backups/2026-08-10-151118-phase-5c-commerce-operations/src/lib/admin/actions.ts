...
export async function updateOrderStatusAction(formData: FormData) {
  ...
  const payload = {
    status,
    carrier: nullableString(formData, "carrier"),
    tracking_number: nullableString(formData, "tracking_number"),
    tracking_url: nullableString(formData, "tracking_url"),
  ...
  await logAdminEvent("order.status_change", "order", id, before, payload);
...
export async function saveCustomerNotesAction(formData: FormData) {
  ...
  const supabase = await createClient();
  const { error } = await supabase.from("customers").update({ notes, tags }).eq("id", id);
  if (error) throw error;
  await logAdminEvent("customer.update", "customer", id, null, { notes, tags });
...
export async function saveSettingsAction(formData: FormData) {
  ...
  const supabase = await createClient();
  const keys = ["shop_name", "support_email", "whatsapp_business_number", "free_shipping_threshold", "default_shipping_cost", "currency", "storefront_enabled"];
  for (const key of keys) {
    const numeric = key.includes("threshold") || key.includes("cost");
    const boolean = key === "storefront_enabled";
